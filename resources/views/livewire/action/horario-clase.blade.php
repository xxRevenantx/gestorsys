    ...
    <div x-data="{
        ...
        @foreach ($grupos as $grupo)
            <div class="mb-6 border rounded-lg">
                    ...
                    <button
                        ...
                        <span class="flex items-center">
                            <svg class="w-5 h-5 me-2 shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 002z" clip-rule="evenodd"></path>
                            </svg>
                            GRUPO: {{ $grupo->grupo }}
                        </span>
                        <svg x-bind:class="{ 'rotate-180': openGroupId == '{{ $grupo->id }}' }"
                             class="w-3 h-3 transition-transform duration-200 shrink-0" aria-hidden="true"
                ...
                <div x-show="openGroupId == '{{ $grupo->id }}'" x-transition id="accordion-open-body-{{$grupo->id}}" aria-labelledby="accordion-open-heading-{{$grupo->id}}">
                        ...
                        <table class="table-auto w-full border-collapse border border-gray-300 mt-4">
                            ...
                            <tbody>
                                ...
                                @if($horario['group_id'] == $grupo->id)
                                    ...
                                    @foreach($dias as $dia)
                                        @php
                                            $materiaId = $horarios[$horario['id']][$dia];
                                            $materia = $materias->firstWhere('id', $materiaId);
                                            $colorFondo = $materia?->teacher?->color ?? '#eee';
                                            $esOscuro = $this->isDarkColor($colorFondo);
                                            $colorTexto = $esOscuro ? 'white' : 'black';
                                        @endphp
                                    @endforeach
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <div class="text-sm mt-1">
                                            Profesor:
                                            {{ $materia?->teacher?->personnel?->nombre ?? 'Sin Profesor' }}
